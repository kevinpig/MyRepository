...
namespace uWS.Dicom.ServiceModel.Query
{
	/// <summary>
	...
	public class StudyRootQueryBridge : IStudyRootQueryBridge
	{
		...
		public IList<StudyRootStudyIdentifier> StudyQuery(StudyRootStudyIdentifier queryCriteria)
		{
			...
			{
			    var list = new List<StudyRootStudyIdentifier>(results);
                list.Sort(_studyComparer.Compare);
			    results = list;
			}
		...
		public IList<SeriesIdentifier> SeriesQuery(SeriesIdentifier queryCriteria)
		{
            ...
            {
                var list = new List<SeriesIdentifier>(results);
                list.Sort(_seriesComparer.Compare);
                results = list;
            }
		...
		public IList<ImageIdentifier> ImageQuery(ImageIdentifier queryCriteria)
		{
            ...
            {
                var list = new List<ImageIdentifier>(results);
                list.Sort(_imageComparer.Compare);
                results = list;
            }
		...
		{
		    return identifiers.Select(identifier => identifier.ToDicomAttributeCollection()).ToList();
		}
