...
namespace uWS.ImageView.Imapi2InteropWrapper.Interop
{
    ...
    internal sealed class DiscFormat2Data_EventProvider : DiscFormat2Data_Event, IDisposable
    {
        ...
        private readonly IConnectionPoint m_connectionPoint = null;
    ...
    {
        // Fields
        private readonly Hashtable _eventSinkHelpers = new Hashtable();
        private readonly IConnectionPoint _connectionPoint = null;
        ...
        public DiscFormat2TrackAtOnce_EventProvider(object pointContainer)
        {
            lock (this)
            {
                ...
                if (connectionPointContainer != null)
                    connectionPointContainer.FindConnectionPoint(ref eventsGuid, out _connectionPoint);
            }
        }
        ...
        public event DiscFormat2TrackAtOnce_EventHandler Update
        {
            add
            {
                lock (this)
                {
                    ...
                    _connectionPoint.Advise(helper, out cookie);
                    helper.Cookie = cookie;
                    _eventSinkHelpers.Add(helper.UpdateDelegate, helper);
                }
            }
            ...
            remove
            {
                ...
                {
                    DiscFormat2TrackAtOnce_SinkHelper helper =
                        _eventSinkHelpers[value] as DiscFormat2TrackAtOnce_SinkHelper;
                    if (helper != null)
                    {
                        _connectionPoint.Unadvise(helper.Cookie);
                        _eventSinkHelpers.Remove(helper.UpdateDelegate);
                    }
                }
            }
        }
        ...
        private void Cleanup()
        {
            ...
            try
            {
                foreach (DiscFormat2TrackAtOnce_SinkHelper helper in _eventSinkHelpers.Values)
                {
                    _connectionPoint.Unadvise(helper.Cookie);
                }

                _eventSinkHelpers.Clear();
    ...
    {
        // Fields
        private readonly Hashtable _evneSinkHelpers = new Hashtable();
        private readonly IConnectionPoint _connectionPoint = null;
        ...
        public event DFileSystemImage_EventHandler Update
        {
            add
            {
                lock (this)
                {
                    ...
                    _connectionPoint.Advise(helper, out cookie);
                    helper.Cookie = cookie;
                    _evneSinkHelpers.Add(helper.UpdateDelegate, helper);
                }
            }
            ...
            remove
            {
                lock (this)
                {
                    DFileSystemImage_SinkHelper helper =
                        _evneSinkHelpers[value] as DFileSystemImage_SinkHelper;
                    if (helper != null)
                    {
                        _connectionPoint.Unadvise(helper.Cookie);
                        _evneSinkHelpers.Remove(helper.UpdateDelegate);
                    }
                }
            }
        }
        ...
        private void Cleanup()
        {
            ...
            try
            {
                foreach (DFileSystemImage_SinkHelper helper in _evneSinkHelpers.Values)
                {
                    _connectionPoint.Unadvise(helper.Cookie);
                }

                _evneSinkHelpers.Clear();
                Marshal.ReleaseComObject(_connectionPoint);
            }
            catch (SynchronizationLockException)
            {
    ...
    internal sealed class DFileSystemImageImport_EventProvider : DFileSystemImageImport_Event, IDisposable
    {
            ...
            remove
            {
                lock (this)
                {
                    DFileSystemImageImport_SinkHelper helper =
                        _eventSinkHelpers[value] as DFileSystemImageImport_SinkHelper;
                    if (helper != null)
                    {
                        _connectionPoint.Unadvise(helper.Cookie);
                        _eventSinkHelpers.Remove(helper.UpdateDelegate);
        ...
        private void Cleanup()
        {
            ...
            try
            {
                foreach (DFileSystemImageImport_SinkHelper helper in _eventSinkHelpers.Values)
                {
                    _connectionPoint.Unadvise(helper.Cookie);
                }
    ...
    internal sealed class DWriteEngine2_EventProvider : DWriteEngine2_Event, IDisposable
    {
        ...
        public event DWriteEngine2_EventHandler Update
        {
            ...
            remove
            {
                ...
                {
                    DWriteEngine2_SinkHelper helper =
                        _eventSinkHelpers[value] as DWriteEngine2_SinkHelper;
                    if (helper != null)
                    {
                        _connectionPoint.Unadvise(helper.Cookie);
                        _eventSinkHelpers.Remove(helper.UpdateDelegate);
        ...
        private void Cleanup()
        {
            ...
            try
            {
                foreach (DWriteEngine2_SinkHelper helper in _eventSinkHelpers.Values)
                {
                    _connectionPoint.Unadvise(helper.Cookie);
                }
